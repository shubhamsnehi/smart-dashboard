...
class WorkspacesBlocks extends React.Component {
    ...
    async getNo() {
        ...
        var tempArry = [];
        for (var i = 0; i < data.dashboard.workspaces.length; i++) {
            // console.log(data.dashboard.workspaces[i].name);
            tempArry[i] = data.dashboard.workspaces[i];
        }
        this.setState({ workspaces: tempArry, loading: false });
    }
        ...
        return (
            <div>
                {this.state.loading ? <div>Loading...</div> :
                    <div class="row">{this.state.workspaces.map((item) =>
...
export default WorkspacesBlocks;
